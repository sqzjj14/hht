...
+ (SecKeyRef)addPublicKey:(NSString *)key{
	...
	data = [UnionpayRSA stripPublicKeyHeader:data];
	if(!data){
		return nil;
	}
...
+ (SecKeyRef)addPrivateKey:(NSString *)key{
	...
	// This will be base64 encoded, decode it.
	NSData *data = base64_decode(key);
	data = [UnionpayRSA stripPrivateKeyHeader:data];
	if(!data){
		return nil;
	}
...
+ (NSString *)encryptString:(NSString *)str privateKey:(NSString *)privKey{
	NSData *data = [UnionpayRSA encryptData:[str dataUsingEncoding:NSASCIIStringEncoding] privateKey:privKey];
...
+ (NSData *)encryptData:(NSData *)data privateKey:(NSString *)privKey{
	...
	SecKeyRef keyRef = [UnionpayRSA addPrivateKey:privKey];
	if(!keyRef){
		return nil;
	}
	return [UnionpayRSA encryptData:data withKeyRef:keyRef];
}
...
+ (NSString *)decryptString:(NSString *)str privateKey:(NSString *)privKey{
	NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
	data = [UnionpayRSA decryptData:data privateKey:privKey];
	NSString *ret = [[NSString alloc] initWithData:data encoding:NSASCIIStringEncoding];
	return ret;
}
...
+ (NSData *)decryptData:(NSData *)data privateKey:(NSString *)privKey{
	...
	return [UnionpayRSA decryptData:data withKeyRef:keyRef];
}
...
+ (NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey{
	NSData *data = [UnionpayRSA encryptData:[str dataUsingEncoding:NSASCIIStringEncoding] publicKey:pubKey];
	NSString *ret = base64_encode_data(data);
	return ret;
}
...
+ (NSData *)encryptData:(NSData *)data publicKey:(NSString *)pubKey{
	...
	SecKeyRef keyRef = [UnionpayRSA addPublicKey:pubKey];
	if(!keyRef){
		return nil;
	}
	return [UnionpayRSA encryptData:data withKeyRef:keyRef];
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey{
    ...
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
	data = [UnionpayRSA decryptData:data publicKey:pubKey];
	NSString *ret = [[NSString alloc] initWithData:data encoding:NSASCIIStringEncoding];
	return ret;
}
...
+ (OSStatus)verifyData:(NSData *)data sig:(NSData *)sig  publicKey:(NSString*) pubKey{
    ...
    SecKeyRef keyRef = [UnionpayRSA addPublicKey:pubKey];
    if(!keyRef) return 0;
    return [UnionpayRSA verifyData:data sig:sig withKeyRef:keyRef];
}
